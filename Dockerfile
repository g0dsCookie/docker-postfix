...
RUN set -eu \
    ...
    && make makefiles shared=yes dynamicmaps=no shlib_directory="/usr/lib/postfix/MAIL_VERSION" \
        DEBUG="" OPT="${CFLAGS}" \
        CCARGS="-DHAS_PCRE -DHAS_LDAP -DHAS_MYSQL -I/usr/include/mysql -DHAS_PGSQL -I/usr/include/postgresql -DHAS_SQLITE -DUSE_TLS -DHAS_LMDB -DDEF_SASL_SERVER=\"dovecot\" -DUSE_LDAP_SASL -DUSE_SASL_AUTH -DUSE_CYRUS_SASL -I/usr/include/sasl -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE -D_LARGEFILE64_SOURCE" \
        AUXLIBS="-ldl -lpcre -llmdb -lssl -lcrypto -lsasl2" \
